...
/**
 * Code for simulating a multi-view acquisition including attenuation,
 * convolution, reduced sampling and poisson noise
 * 
...
public class SimulateMultiViewDataset
{
	...
	public static AffineModel3D axisRotation(	final Interval in,
																						final int axis,
																						final int degrees)
	{
	...
	public static Img<FloatType> rotateAroundAxis(final RandomAccessibleInterval<FloatType> in,
																								final int axis,
																								final int degrees)
	{
		// final model
		final AffineModel3D affine = axisRotation(in,
																							axis,
																							degrees).createInverse();

		final Img<FloatType> out = new ArrayImgFactory<FloatType>().create(	in,
																																				new FloatType());

		final NLinearInterpolatorFactory<FloatType> factory =
																												new NLinearInterpolatorFactory<FloatType>();
		final RealRandomAccessible<FloatType> interpolant =
																											Views.interpolate(Views.extendZero(in),
																																				factory);
	 ...
	 * @return
	 */
	public static Img<FloatType> makeIsotropic(	final RandomAccessibleInterval<FloatType> randomAccessible,
																							final int inc)
	{
		final long[] dim = new long[]
		{	randomAccessible.dimension(0),
			randomAccessible.dimension(1),
			(randomAccessible.dimension(2) - 1) * inc + 1 };
		final Img<FloatType> img = new ArrayImgFactory<FloatType>().create(	dim,
																																				new FloatType());

		final NLinearInterpolatorFactory<FloatType> factory =
																												new NLinearInterpolatorFactory<FloatType>();
		final RealRandomAccessible<FloatType> interpolant =
																											Views.interpolate(Views.extendMirrorSingle(randomAccessible),
																																				factory);
		final RealRandomAccess<FloatType> realRandomAccess = interpolant.realRandomAccess();
	...
	/**
	 * Scans the sample with a simulated lightsheet ... the width of the
	 * lightsheet is implicitly defined by the effective PSF
	 * 
	 * @param randomAccessible
	 * @param inc
	 *          - every n'th
	 * @param poissonSNR
	 *          - which poisson SNR is desired?
	 * @return
	 */
	public static Img<FloatType> extractSlices(	final RandomAccessibleInterval<FloatType> randomAccessible,
																							final int inc,
																							final float poissonSNR)
	{
		if ((randomAccessible.dimension(2) - 1) % inc != 0)
			throw new RuntimeException("Index of last z slice needs to be divisable by "
																	+ inc);
		...
		{
			final RandomAccessibleInterval<FloatType> slice =
																											Views.hyperSlice(	randomAccessible,
																																				2,
																																				z);
	...
	public static Img<FloatType> poissonProcess(final RandomAccessibleInterval<FloatType> in,
																							final float poissonSNR)
	{
		final Img<FloatType> out = new ArrayImgFactory<FloatType>().create(	in,
																																				new FloatType());
	...
	public static Img<FloatType> convolve(final Img<FloatType> img,
																				final Img<FloatType> psf)
	{
		...
		final FFTConvolution<FloatType> conv = new FFTConvolution<FloatType>(	img,
																																					psf,
																																					result);
		// this fixes the wrong default setting in FFTConvolution
	...
	public static Img<FloatType> computeWeightImage(final RandomAccessibleInterval<FloatType> randomAccessible,
																									final double delta)
	{
		...
		// the weight image
		final Img<FloatType> img = new ArrayImgFactory<FloatType>().create(	randomAccessible,
																																				new FloatType());
		...
		while (c.hasNext())
		{
			...
			{
				final double pos = ((double) (l - sizeY / 2)
														/ (double) cosineSpan) * Math.PI;
	...
	public static Img<FloatType> attenuate3d(	final RandomAccessibleInterval<FloatType> randomAccessible,
																						final double delta)
	{
		...
		// make a plane that only contains x & z, this is the origin for the
		// attenuation along y
		final RandomAccessibleInterval<FloatType> startMatrix =
																													Views.hyperSlice(	randomAccessible,
																																						1,
																																						0);
		final Cursor<FloatType> c = Views	.iterable(startMatrix)
																			.localizingCursor();

		final RandomAccess<FloatType> rIn =
	...
	public static Img<FloatType> simulate()
	{
		...
		Img<FloatType> img = new ArrayImgFactory<FloatType>().create(new long[]
		{ size * scale, size * scale, size * scale }, new FloatType());
	 ...
	 *
	 * @param randomAccessible
	 *          - the image data to write to
	 * @param minValue
	 *          - the minimal intensity of one of the small spheres
	 * @param maxValue
	 *          - the maximal intensity of one of the small spheres
	 */
	public static <T extends RealType<T>> void drawSpheres(	final RandomAccessibleInterval<T> randomAccessible,
																													...
																													final int scale)
	{
		...
		HyperSphere<T> hyperSphere = new HyperSphere<T>(randomAccessible,
																										center,
																										radiusLargeSphere);
		...
		while (cursor.hasNext())
		{
			...
			HyperSphere<T> smallSphere = new HyperSphere<T>(randomAccessible,
																											cursor,
																											radius);
			...
			// take only every 4^dimension'th pixel by chance so that it is not too
			// crowded
			if (Math.round(randomValue * 10000)
					% Util.pow(7 * scale, numDimensions) == 0)
			{
				// scale to right range
				randomValue = rnd.nextDouble() * (maxValue - minValue)
											+ minValue;
	...
	public static void main(String[] args)
	{
		...
		// including the ground-truth image, which is rotated once by the angle
		// offset
		// so that it is realistic
		System.out.println(new Date(System.currentTimeMillis())
												+ ": rendering basis for ground truth");
		final Img<FloatType> rendered = simulate();

		System.out.println(new Date(System.currentTimeMillis())
												+ ": computing ground truth");
		final Img<FloatType> obj = rotateAroundAxis(rendered,
																								0,
																								angleOffset);
		...
		final ArrayList<Img<FloatType>> weights =
		...
		for (int angle = 0; angle < 360; angle += angleIncrement)
		{
			...
			Img<FloatType> rot = rotateAroundAxis(rendered,
																						0,
																						angle + angleOffset);
			...
			System.out.println(new Date(System.currentTimeMillis())
													+ ": convolving angle " + angle);
			Img<FloatType> psf = Tools.open(dir	+ "Angle"
																			+ angle
																			+ ".tif",
																			true);
			...
			System.out.println(new Date(System.currentTimeMillis())
													+ ": extracting slices angle " + angle);
			Img<FloatType> acq = extractSlices(	con,
																					lightsheetSpacing,
																					poissonSNR);

			System.out.println(new Date(System.currentTimeMillis())
													+ ": make isotropic angle " + angle);
			Img<FloatType> iso = makeIsotropic(acq, lightsheetSpacing);

			System.out.println(new Date(System.currentTimeMillis())
														+ ": rotate back view, weights and psf angle "
													+ angle);
		...
		for (int i = 0; i < weights.size(); ++i)
		{
			...
			Tools.save(	weights.get(i),
									dir + "aligned_view_weights" + angle + ".tif");
		}

		final Img<FloatType> sumWeights =
																		weights	.get(0)
																						.factory()
																						.create(weights.get(0),
																										weights	.get(0)
																														.firstElement());
